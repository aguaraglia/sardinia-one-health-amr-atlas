...
$rows = foreach ($id in $ids) {
    ...
    [pscustomobject]@{
        ...
        status = if ($curatedPmids.ContainsKey($id)) { 'already_curated' } else { 'needs_manual_review' }
        reason = if ($curatedPmids.ContainsKey($id)) { 'Already present in metadata/LITERATURE_CURATED.tsv.' } else { 'Discovery candidate only; scope, Sardinia-specificity, data granularity and public-safety must be screened manually.' }
